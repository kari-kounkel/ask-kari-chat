    ...
    "<div id='ak-intro'>",
      (SUPPORT
        ? [
            ...
            "<input id='ak-se' type='email' placeholder='Email address' style='margin-bottom:8px;'/>",
            "<textarea id='ak-q' rows='4' placeholder='What do you need help with?'></textarea>",
            "<div id='ak-err'>Name, email, and question, please.</div>",
  ...
  document.getElementById("ak-go").addEventListener("click", async function() {
    try {
      if (SUPPORT) {
        ...
        var se = (document.getElementById("ak-se").value || "").trim();
        var sq = (document.getElementById("ak-q").value || "").trim();
        var serr = document.getElementById("ak-err");
        if (!sq || !sn || !/.+@.+\..+/.test(se)) { serr.style.display = "block"; return; }
        serr.style.display = "none";
        var sres = await fetch(SURL + "/rest/v1/conversations", {
          method: "POST",
          headers: Object.assign({}, H, { "Prefer": "return=representation" }),
          body: JSON.stringify({ visitor_id: "v_" + Date.now(), visitor_name: sn || CFG.user || "Guest", visitor_email: se, site_origin: CFG.site || window.location.origin, type: "support", status: "open" })
        });
